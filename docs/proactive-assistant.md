...
离线主动事件使用 5 秒起步、最长 5 分钟的指数退避。通道未开启时，单一 FreeRTOS worker 在确认网络已连接、设备空闲且无其他连接 worker 后执行 `OpenAudioChannel`。独立原子 `busy` 是跨任务协议门禁与析构等待的唯一权威：主线程在创建 worker 前发布 true，创建失败才回滚；worker 全程保持 true，并以二值信号量 Give 作为最后一次 Application 访问，返回后仅由任务入口执行 `vTaskDelete`。Finish 取得完成信号后才由主线程清空 handle 和 busy；析构看到 busy=true 就无条件等待。Start 与析构均只在主任务运行，因此不存在析构插入 busy 发布与任务创建之间的窗口。双核极短窗口内若 Finish 尚未取到信号，只记录一次待完成状态并由下一时钟 tick 重试，不自旋。NVS 主动状态预算为 4×1800=7200 字节，覆盖完整合法上限。保存预检使用全局 `available_entries` 加 proactive 命名空间 `nvs_get_used_entry_count` 的可回收旧 entry，并要求覆盖新状态最终 entry、单个最大 chunk 的替换峰值和 16 entry 安全余量；因此首存以及已有 4 chunk 时的同等、缩小、扩大重写都可通过，而其他命名空间真实挤占过高仍会拒绝。统计、写入或提交失败会明确日志、本地告警并保持内存重试。
