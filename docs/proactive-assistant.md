...
网络抖动只统计 `Connected → Disconnected` 的真实转换，扫描和初始未连接状态不计数；转换时用单调时钟记录，事件位合并不会丢失次数。健康提示仅在设备空闲且本地播放队列空闲时播放 popup 音，不中断闹铃、提醒或正在播报的内容。健康事件保留严重级别映射：info 为 normal、warning 为 high、critical 为 critical，只有 critical health 绕过普通 mode、quiet、预算和冷却；恢复通知走独立旁路。同一 priority 下健康事件优先保留在主队列；被更高优先级事件淘汰或暂不能入队的健康事件按 dedupe key 合并到最多 8 项的持久待重试槽，覆盖当前 network/time/OTA/audio 故障与恢复组合。槽满时拒绝新项并持续显示本地错误，不再淘汰旧健康事件。恢复事件入队前同时删除主队列和待重试槽中相同 dedupe key 的旧故障，避免恢复后再次上报旧状态。

离线主动事件使用 5 秒起步、最长 5 分钟的指数退避。通道未开启时，单一 FreeRTOS worker 在确认网络已连接、设备空闲且无其他连接 worker 后执行 `OpenAudioChannel`；worker 优先级高于主任务，排入完成回调后会先发出完成信号并退出，主线程取得信号前不清除协议独占标记。双核极短窗口内若完成信号尚未可取，主线程只记录一次待完成状态并由下一时钟 tick 重试，不自调度自旋。worker 期间主线程的建链、发送、查询和关闭操作只登记延后，不与 worker 并发；外部任务提交唤醒词时也只把具体字符串调度到主线程。worker 不进入 listening、不发送通知，完成后的下一次 tick 才发送队列事件。ResetProtocol 等 worker 完成后再释放协议；析构设置共享 shutdown 标记并无限等待 worker 的完成信号，已排队回调检查标记后不再访问 Application。NVS 主动状态采用每片 1800 字节、总计最多 3600 字节的明确预算，最多保存 4 项完成追踪、8 项主队列事件、8 项健康待重试事件和 8 项主题规则，并在写入前用 NVS entry 统计保留 16 个安全 entry。保存失败会记录上下文、显示本地错误并按退避重试；MCP 修改则回滚内存并返回明确失败。
