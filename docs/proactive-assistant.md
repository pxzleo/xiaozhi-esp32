...
离线主动事件使用 5 秒起步、最长 5 分钟的指数退避，只有退避到期才尝试复用音频通道建链；不会每秒同步重连。NVS 主动状态采用每片 1800 字节、总计最多 3600 字节的明确预算，最多保存 4 项完成追踪、8 项队列事件和 8 项主题规则，并在写入前用 NVS entry 统计保留 16 个安全 entry，避免占满目标板仅 16KB 的共享 NVS 分区或由 `ESP_ERROR_CHECK` 触发复位。超限会返回或记录明确错误，不扩大分区、不伪造文件系统可用空间；该 entry 预检只服务于主动状态原子保存，不等同于跨目标板的 `storage_low` 指标。
