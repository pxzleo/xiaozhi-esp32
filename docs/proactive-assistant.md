...
网络抖动只统计 `Connected → Disconnected` 的真实转换，扫描和初始未连接状态不计数；转换时用单调时钟记录，事件位合并不会丢失次数。健康提示仅在设备空闲且本地播放队列空闲时播放 popup 音，不中断闹铃、提醒或正在播报的内容。所有健康事件在设备持久队列中使用 critical 优先级；普通/follow-up 事件不能淘汰健康事件，同级满时新健康事件进入按 dedupe key 合并的内存重试队列。恢复事件替换相同 dedupe key 的未发送故障事件，并绕过普通 mode、quiet、预算和冷却，确保恢复最终上报。

离线主动事件使用 5 秒起步、最长 5 分钟的指数退避。通道未开启时，单一 FreeRTOS worker 在确认网络已连接、设备空闲且无其他连接 worker 后执行 `OpenAudioChannel`；完成结果调度回主线程，worker 不进入 listening、不发送通知。主任务绝不调用该同步建链函数，ResetProtocol 会等待连接 worker 结束后再释放协议，析构也等待 worker 退出。建链成功后的下一次 tick 才发送队列事件，因此 10 分钟追问即使等待 120 秒通道恢复，仍在到期后 5 分钟有效窗口内继续。NVS 主动状态采用每片 1800 字节、总计最多 3600 字节的明确预算，最多保存 4 项完成追踪、8 项队列事件和 8 项主题规则，并在写入前用 NVS entry 统计保留 16 个安全 entry，避免占满目标板仅 16KB 的共享 NVS 分区。保存使用可返回错误的直接 NVS 操作，不经过会 `ESP_ERROR_CHECK` 复位的 Settings 写路径；内部事件保存失败会记录上下文、显示本地错误并按退避重试，MCP 配置和完成操作则回滚内存并返回明确失败。超限不会扩大分区或伪造文件系统可用空间；该 entry 预检只服务于主动状态原子保存，不等同于跨目标板的 `storage_low` 指标。
