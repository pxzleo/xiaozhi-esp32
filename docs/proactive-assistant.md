...
- 外界事件专用 WebSocket 建连期间，服务端发起的 MCP 初始化与工具列表请求属于正常握手；设备先按 FIFO 补发握手响应，再继续保持外界连接用途并发送 `notifications/assistant/external_triggered`。只有用户动作、网络关闭、协议重置、重启或建连失败才抢占外界投递，不能因握手响应到达时序不同而清除待播事件。
