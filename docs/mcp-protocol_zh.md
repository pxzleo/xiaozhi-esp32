...
离线重试采用 5 秒到 5 分钟的指数退避；音频通道由高于主任务优先级的单 FreeRTOS worker 自动建立，主任务不调用 `OpenAudioChannel`。独立原子 busy 在任务创建前发布，并在 worker Give 完成信号前始终为 true；Give 是 worker 最后一次 Application 访问，Finish 取到信号后才由主线程清 busy/handle，析构看到 busy 就无条件等待。外部唤醒词始终携带具体字符串调度到主线程。健康事件只接受 network/time/OTA/audio 四类 kind，活动与恢复按 dedupe key 在统一 8 项主队列中替换。主动状态 NVS 预算为 4×1800=7200 字节；容量预检按全局可用 entry + proactive 旧 entry 计算，并预留单最大 chunk 替换峰值与 16 entry 安全余量，允许正常重复保存但仍拒绝真实全局空间不足。NVS 故障明确告警并退避重试。
