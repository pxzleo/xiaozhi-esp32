...
- `self.proactive.allow_topic(topic)` / `self.proactive.block_topic(topic)`：更新主题规则；topic 只允许 `reminder/calendar/weather/music/health/habit/system`，allow 集合非空时是普通事件白名单，critical 不受白名单限制。
